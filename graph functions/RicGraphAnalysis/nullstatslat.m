function [realvsRN, eigenvector_mean_ratio, eigenvector_max_ratio, num_rich_club_nodes, num_hubs ,num_hubs_RN_ratio, assortativity_ratio, modularity_ratio] = nullstatslat(adj_matrix)
...
ave_deg_real = mean(nonzeros(degreal));
std_deg_real = std(nonzeros(degreal));
var_de_real = std_deg_real^2;
max_deg_real = max(degreal);

%crate a null-model with Watts Strogtz rewiring algorithm
WS = WattsStrogatz(num_nodes_real,round(ave_deg_real/2),0);
...
Cadj =clustering_coef_bu(adj_matrix);
Crn = clustering_coef_bu(RN_adj);
Clat = clustering_coef_bu(WS_adj);

Cratiorn = (mean(Cadj)/mean(Crn));
Cratio = (mean(Cadj) - mean(Crn))/(mean(Clat) - mean(Crn));
...
eigLT = eigenvector_centrality_und(WS_adj);
eigenvector_mean_ratio = (mean(eigreal) - mean(eigRN))/(mean(eigLT) -mean(eigRN));
eigenvector_max_ratio = (max(eigreal) - max(eigRN))/(max(eigLT) - max(eigRN));
...
elseif mean(nonzeros(adj_matrix(:)))== 1 
    ...
    assortativity_RN = assortativity_bin(RN_adj,0);
    assortativity_LT = assortativity_bin(WS_adj,0);
    assortativity_ratio = (assortativity_real - assortativity_RN)/...
        (assortativity_LT - assortativity_RN);
end

%modularity ratio
[Cireal,Qreal]=modularity_und(adj_matrix,1.1);
[CiRN,QRN]=modularity_und(RN_adj,1.1);
[CiLT,QLT] = modularity_und(WS_adj, 1.1);
modularity_ratio = (Qreal-QRN)/(QLT - QRN);
